...
class G3DBaseExporterOperator(ExportHelper, IOG3DOrientationHelper):
	...
	combineAllAction = BoolProperty(
		name="Combine All Action To One Animation",
		description="For libgdx control Animation Play",
		default=True,
	)
	...
	def generateMaterials(self, context):
		...
		if bpy.data.materials is not None and len(bpy.data.materials) > 0:
			...
			for blMaterial in bpy.data.materials:
				...
				currentMaterial.emissive = [blMaterial.emit * blMaterial.diffuse_color[0] / 2 \
											, blMaterial.emit * blMaterial.diffuse_color[1] / 2 \
											, blMaterial.emit * blMaterial.diffuse_color[2] / 2] \
											if hasattr(blMaterial,'emit') else [0.0, 0.0, 0.0]
	...
	def generateAnimations(self, context):
		...
		if self.exportAnimation:
			...
			for blAction in bpy.data.actions:
				...
				if self.combineAllAction:
					currentAnimation.id = 'Anim'
					onlyOneAnim = True
				else:
					currentAnimation.id = blAction.name
				
				for blArmature in bpy.data.objects:
					if hasattr(blArmature.animation_data,'action'):
						if blArmature.animation_data.action.name != blAction.name:
							if not self.combineAllAction:
								continue
					else:
						continue
					...
					currentBone.boneId = blBone.name
